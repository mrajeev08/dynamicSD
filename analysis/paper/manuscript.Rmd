...
if(!params$diss) {
cat("Authors: Malavika Rajeev, Isty Rysava, Elaine Ferguson, Rebecca Mancy, Anna Czupryna, Ahmed Lugelo, + others in TZ + Jess + Katie \n

Analysis plan: \n
- For diss sub: updated parameter recovery and figs per KH \n
-	Fixing incursions so they seed at least one case per Rebecca? \n
-	Exploring and visualizing endemic dynamics in parameter space per FPO \n
- Exploring and visualizing trees/chains empirical vs. simulated per FPO \n
-	Relating district level coverage to village heterogeneity and generally exploring whether there are predictors of outbreak probability for a given campaign scenario (connectivity, village identity, etc?) \n
-	Sensitivity analyses (for best models only) \n
-	use strong priors on the fixed parameters (i.e. dispersal, serial interval) and assess sensitivity of parameters estimates \n
-	use SMC to get joint parameter estimates \n
")
}
...
2)	3)	A full household and dog census of the district conducted between 2009 - 2014, with the spatial location of households, the number of people and of adult dogs and puppies in each household, and their vaccination status (\@ref(fig:data)C).
...
tab1 <-
  tribble(
    ...
    "$R_{0}$ and $k$ are estimated, prior centered around estimates from (ref:tabhamps)", 
    "Weekly rate of introductions", "Poisson distribution", "Estimated", 
    "Serial interval distribution (days)", 
    glue("Lognormal distribution with mean = {round(param_defaults$serial_meanlog, 2)}, and sd = {round(param_defaults$serial_sdlog, 2)}, for a mean interval of approximately {round(mean(serial_lognorm(1000, param_defaults)))} days"), 
    "(ref:tabmancy)", 
    "Dispersal kernel (meters)", 
    glue("Lognormal distribution with mean = {round(param_defaults$disp_meanlog, 2)}, and sd = {round(param_defaults$disp_sdlog, 2)}, for a mean dispersal length of approximately {round(mean(dispersal_lognorm(1000, param_defaults)))} meters"), 
    "(ref:tabmancy)", 
    "Step length distribution (meters)",
    glue("Convolved Weibull distribution with shape = {round(steps_conv$estimate['shape'], 2)*2}, and scale = {round(steps_conv$estimate['scale'], 2)*2}, for a mean step length of approximately {round(mean(conv), 2)} meters"), 
    "(ref:tabmancy)", 
     "Detection probability (monthly)", 
    glue("Beta binomial distribution with $\\beta$ = {round(param_defaults$detect_beta, 2)}, $\\alpha$ = {round(param_defaults$detect_alpha, 2)}, for a mean monthly detection probability of approximately {round(mean(rbeta(1e4, param_defaults$detect_alpha, param_defaults$detect_beta)), 3)}"), 
    "(ref:tabmancy)", 
    "Annual death rate (dogs/yr)", "1/25.8 yrs", "(ref:tabczup)", 
    "Rate of waning vaccine immunity (dogs/yr)", "1/3 yrs", "(ref:tabhamps)"
  )

if(knitr::is_latex_output()) {
  kbl(tab1, booktabs = TRUE, escape = FALSE, 
      caption = "Table of parameter inputs and sources.") %>% 
    kable_styling(full_width = TRUE) %>% 
    column_spec(1, width = "10em")
} else {
  kable(tab1, caption = "Table of parameter inputs and sources.", 
        format = "pipe")
}

```

(ref:tabczup) @czupryna2016

(ref:tabhamps) @Hampson2009

(ref:tabmancy) @Mancyinprep


### Model selection & parameter estimation
We use Random Forest Approximate Bayesian Computation (RF-ABC) implemented in the R package `abcrf`  [@pudlo2015; @raynal2018] to perform model selection and parameter inference. We estimate three parameters: $R_{0}$ (the mean of the secondary case distribution), $k$ (the dispersion parameter of the secondary case distribution), and $\iota$ (the mean weekly number of introductions of rabies cases from outside of the district). We use strong priors based on previous analyses of part of this dataset (Table \@ref(tab:tbinputs))[@Hampson2009]. RF-ABC can be used to evaluate both models and potential summary statistics (i.e. key features of the data that the model should capture). Table \@ref(tab:tbsstats) lists the candidate summary statistics we used for model selection and parameter estimation, encompassing temporal, spatial, and spatiotemporal metrics of the data. RF-ABC ranks each of these parameters by their importance (i.e. Gini importance, specifically how frequently a variable is used for node classification and broadly a relative measure of the discriminative value a statistic has for the classification problem) in predicting a given parameter or in differentiating models. For both model selection and parameter estimation, we generate N = 1e5 simulated sets of summary statistics. To manage computational resources, we stop any simulations where the host population has declined to greater than 15% of the starting population, as these simulations generally lead to extinction of domestic dogs and represent unlikely scenarios. We include these simulations in the dataset, but calculate summary statistics only including the data before the simulation was stopped. 
...
```{r tbsstats}
sstat_tab <- readr::read_csv(here("analysis/out/summ_stats_labs.csv"))

if(knitr::is_latex_output()) {
  kbl(dplyr::select(dplyr::filter(sstat_tab, !is.na(descr)), -summstat), 
             col.names = c("Summary statistic", "Description", "Type"),
    booktabs = TRUE, escape = FALSE, 
    caption = "Summary statistics used for fitting in Random Forest ABC.") %>% 
  kable_styling(full_width = TRUE) %>% 
  column_spec(2, width = "20em")
} else {
  kable(dplyr::select(dplyr::filter(sstat_tab, !is.na(descr)), -summstat), 
        col.names = c("Summary statistic", "Description", "Type"),
        caption = "Summary statistics used for fitting in Random Forest ABC.", 
        format = "pipe")
}
...
1) 'Chopping the tail' of the secondary case distribution [@Kain2020], where we decrease the censoring value as vaccination increases, to approximate how enhanced surveillance and isolation/quarantine/removal of rabid dogs can improve control outcomes
...
par_ests <- read_csv(here("analysis/out/par_stats_comp.csv"))

par_ests %>%
  filter(interaction(move, incs, scale, limits) %in% cnames, type == "full") %>%
  mutate(across(where(is.numeric), ~round(.x, 2))) %>%
  mutate(text_val = glue("{mean} ({quantile_0.025} - {quantile_0.975})")) %>%
  dplyr::select(scale, par_type, text_val, param) %>%
  tidyr::pivot_wider(names_from = par_type, values_from = text_val, 
                     id_cols = c("scale", "param")) %>%
  mutate(param = case_when(param == "R[0]" ~ "$R_{0}$", 
                           param == "iota" ~ "$iota$", 
                           param == "k" ~  "$k$")) %>%
  dplyr::select(scale, param, Prior, Independent, Joint) -> out_pars

if(knitr::is_latex_output()) {
  kbl(out_pars, 
    col.names = c("Scale", "Parameter", "Prior", "Independent posterior", "Joint posterior"),
    booktabs = TRUE, escape = FALSE,
    caption = 'Priors and independent and joint posterior parameter estimates (mean with 95\\% quantiles in parentheses) for the best model at each scale.') %>% 
  kable_styling(full_width = TRUE) %>% 
  column_spec(1, width = "10em")
} else {
  kable(out_pars, 
    col.names = c("Scale", "Parameter", "Prior", "Independent posterior", "Joint posterior"),
        caption = 'Priors and independent and joint posterior parameter estimates (mean with 95% quantiles in parentheses) for the best model at each scale.', format = "pipe")
}
...
```{r mfig-posts-vimp, fig.cap='(ref:mfig-posts-vimp-cap)', out.width="100%", fig.scap="Ranking of summary statistics for importance in predicting each parameter."}
knitr::include_graphics(here("analysis/figs/mfig_posts_vimp.jpeg"))
```


(ref:mfig-posts-vimp-cap) Ranking of summary statistics for importance in predicting each parameter. Summary statistics are colored by the type of statistic (either spatial, temporal, or spatial temporal). Results are shown for the best fitting model at the village and 1 km scale.


```{r mfig-posts-best, fig.cap='(ref:mfig-posts-best-cap)', fig.scap='Independent and joint posterior parameter estimates for the best fitting model across scales.'}
knitr::include_graphics(here("analysis/figs/mfig_posts_best.jpeg"))
```


(ref:mfig-posts-best-cap) Joint posterior estimates compared to prior for the best fitting model at the village and 1 km scale for the three parameters, with prior density in grey.
...
```{r mfig-sims-best, fig.cap='(ref:mfig-sims-best-cap)', fig.scap="Simulations from the joint posterior distributions of the best fitting model."}
...
```{r sfig-mod-ranks, fig.cap='(ref:sfig-mod-ranks-cap)', fig.scap='Posterior probability of models given observed data.'}

knitr::include_graphics(here("analysis/figs/sfig_mod_ranks.jpeg"))
```


(ref:sfig-mod-ranks-cap) Posterior probability of each model given our observed data, with the panels comparing models with fixed vs. estimated introductions, the colors showing the model scale, and the x-axis how movement was specified (see Methods).


```{r sfig-mod-vimp, fig.cap='(ref:sfig-mod-vimp-cap)', fig.scap='Variable importance plots for parameters with full or subsampled simulations.'}
...
```{r sfig-posts-vimp, fig.cap='(ref:sfig-posts-vimp-cap)', fig.scap='Variable importance plots for parameters with full or subsampled simulations.'}
...
```{r sfig-posts-jntvsfull, fig.cap='(ref:sfig-posts-jntvsfull-cap)', fig.scap='Independent posterior estimates for models run with the full simulation data set'}
...
```{r sfig-par-recov, fig.cap='(ref:sfig-par-recov-cap)'}

knitr::include_graphics(here("analysis/figs/sfig_param_recov.jpeg"))
```


(ref:sfig-par-recov-cap) Simulated (true) parameter values vs. model estimates for 1000 simulated datasets witheld from the classification model. 


```{r sfig-sims-full, fig.cap='(ref:sfig-sims-full-cap)', fig.scap='Simulations from the independent posterior estimates for all the models.'}

knitr::include_graphics(here("analysis/figs/sfig_sims_full.jpeg"))
```


(ref:sfig-sims-full-cap) Simulations from the independent posterior estimates for all the models. The grey envelope shows the range of simulations, and the black line is the time series of observed monthly cases. The orange lines show the top five simulations that best fit the data (lowest RMSE) and the purple lines show the top five simulations that have the highest centrality score.


```{r sfig-sims-joint, fig.cap='(ref:sfig-sims-joint-cap)', fig.scap='Simulations from the joint posterior estimates for all the models.'}
...
(ref:sfig-sims-crps-cap)  Continuous ranked probability scores for the simulated datasets compared to the observed comparing models with estimated introduction rates (columns) and across scales (rows).
...
(ref:sfig-base-mets-cap)  Simulation outcomes across a range of campaign coverage and spatial coverage for the baseline case where panels are different summary statistics of the simulations (i.e. chain length mean, chain length peak, etc.).


```{r sfig-sspread-mets, fig.cap='(ref:sfig-sspread-mets-cap)', fig.scap='Simulation outcomes across a range of campaign coverage and spatial coverage.'}

knitr::include_graphics(here("analysis/figs/sfig_sspread_mets.jpeg"))
```


(ref:sfig-sspread-mets-cap)  Simulation outcomes across a range of campaign coverage and spatial coverage incorporating reductions in the maximum number of secondary cases rate as district coverage increases where panels are different summary statistics of the simulations (i.e. chain length mean, chain length peak, etc.).
...
(ref:sfig-incs-mets-cap)  Simulation outcomes across a range of campaign coverage and spatial coverage incorporating reductions in the introduction rate as district coverage increases where panels are different summary statistics of the simulations (i.e. chain length mean, chain length peak, etc.).
...
(ref:sfig-conn-mets-cap)  Outbreak durations (weeks with cases > 95th % of introductions) and their relationship to connectivity of villages across a range of campaign coverages (i.e. proportion vaccinated in each village, x-axis) and spatial coverage (proportion of villages vaccinated, panels).
