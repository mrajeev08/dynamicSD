...
get_name <- function(i) {
  ...
  move <- ifelse(weights, "moveprob", "movecont")
  
  paste(scale, move, est_incs, seq, sep = "_")
...
out_times <- foreach(i = iter(cand, by = "row"), .combine = rbind) %do% {
  ...
  foreach(j = seq_len(nsims), .combine = 'rbind', 
        ...
        c(simstats, R0 = R0_vals[j], k = k_vals[j], iota = iota_vals[j])
    } -> sims
  
  file_out <- paste0("analysis/out/fit/", get_name(i))
