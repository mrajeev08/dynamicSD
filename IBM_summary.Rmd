...
```{r}
perc_cell <- read.csv("output/perc_df.csv")
perc_cell %>%
  group_by(perc, cov) %>%
  summarize(chain_length = mean(chain_length), secondaries = mean(secondaries)) -> perc_cell
ggplot(data = perc_cell, aes(x = cov, y = perc, fill = secondaries)) +
  geom_tile() +
